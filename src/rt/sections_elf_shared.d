...
version (Shared)
{
     ...
     * null when druntime is dynamically loaded by a C host, or on OS X, where
     * there are no copy relocations.
     */
        ...
        *data._slot = pdso; // store backlink in library record
        ...
        version (Shared)
        {
            ...
            static if (SharedELF)
            {
                ...
                // FIXME: Use hash table on OS X.
                checkModuleCollisions(header, pdso, _copyRelocSection);
            }
 ...
 * chains.
 */
version (Shared) static if(SharedELF)
void checkModuleCollisions(in ref ImageHeader info, in DSO* pdso,
...
body
{
    ...
    if (conflicting !is null)
    {
        ...
        auto existing = dsoName(nameForAddr(conflicting));
